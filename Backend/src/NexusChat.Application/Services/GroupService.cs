...
public class GroupService(
    ...
    IRealtimeNotification notify) : IGroupService
{
    public async Task<ErrorOr<GroupResponseDto>> CreateGroupAsync(string creatorId, CreateGroupRequestDto dto,
        CancellationToken token)
    {
        var participants = new List<Participant>
        {
            new() { UserId = creatorId, Role = ParticipantRole.Owner, JoinedAt = DateTime.UtcNow },
        };
    ...
    public async Task<ErrorOr<GroupResponseDto>> UpdateGroupAsync(string userId, string groupId,
        UpdateGroupRequestDto dto, CancellationToken token)
    {
    ...
    public async Task<ErrorOr<GroupResponseDto>> AddMemberAsync(string userId, string groupId, AddMemberRequestDto dto,
        CancellationToken token)
    {
        var group = await conversationRepo.GetByIdAsync(groupId, token);
        if (group is null || group.RoomType != RoomType.Group)
        ...
        if (dto.ParticipantIds.Any())
        {
            var isMember = group.Participants.Any(p => p.UserId == userId);
            if (!isMember) return Error.Validation("Group.Unauthorized", "Bạn không phải là thành viên của nhóm này.");
        }
    ...
    public async Task<ErrorOr<bool>> RemoveMemberAsync(string actionUserId, string groupId, string targetUserId,
        CancellationToken token)
    {
        var group = await conversationRepo.GetByIdAsync(groupId, token);
        if (group is null || group.RoomType != RoomType.Group)
