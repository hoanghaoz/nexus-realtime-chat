...
public static class MongoIndexConfig
{
    public static async Task CreateIndexAsync(IMongoDatabase database)
    {
        ...
        var conversationIndexModel = new CreateIndexModel<Conversation>(
            conversationIndexKeys,
            indexConversationOptions
        );
        ...
        var parentMessageIndexModel = new CreateIndexModel<Message>(
            parentMessageIndexKeys,
            indexParentMessageOptions
        );
        ...
        var messageReplyIndexModel = new CreateIndexModel<Message>(
            messageReplyIndexKeys,
            messageReplyIndexOptions
        );
        ...
        var friendRequestToIndexModel = new CreateIndexModel<FriendRequest>(
            friendRequestToIndexKeys,
            indexFriendRequestToOption
        );

        var friendRequestFromIndexModel = new CreateIndexModel<FriendRequest>(
            friendRequestFromIndexKeys,
            indexFriendRequestFromOption
        );

        await friendRequest.Indexes.CreateManyAsync([
            friendRequestToIndexModel,
            friendRequestFromIndexModel
        ]);

        // Participants index
        var participant = database.GetCollection<Participants>("Participant");

        var participantIndexKeys = Builders<Participants>.IndexKeys
        ...
        var indexParticipantModel = new CreateIndexModel<Participants>(
            participantIndexKeys,
            indexParticipantOptions
        );
        ...
        var indexReminderModel = new CreateIndexModel<Reminder>(
            reminderIndexKeys,
            indexReminderOptions
        );
